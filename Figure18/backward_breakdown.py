...
from efficient_tt_backward import Eff_TTEmbedding
...
if __name__ == "__main__":
    ...
    for i in range(100):
        ...
        for idx in tt_index:
            index = sparse[idx]
            eff_emb = emb_list[k]
            output = eff_emb(index, offset, fuse)
            fake_y = torch.randn(output.shape).to(device)
            Loss = loss_fn(fake_y, output)
            Loss.backward()
            k += 1
