...
from modules.dlrm_modules import Single_Large_Table
from modules.dlrm_modules import DLRM
...
#         logits =  embedded_sparse.sum((1,2)).unsqueeze(1)
#         logits = 1/logits
...
class DLRMTrain(nn.Module):
    ...
    def forward(
        self, batch: Batch, inference: bool = False
...
class Single_Table_Train(nn.Module):
    def __init__(
        self,
        embedding_bag_collection: EmbeddingBagCollection,
        feature_dim: int,
    ...
    def forward(
        ...
        logits = self.model(batch.sparse_features)
        logits = logits.squeeze()
        loss = self.loss_fn(logits, batch.labels.float())
