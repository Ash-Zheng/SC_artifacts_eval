...
torch::Tensor rabbit_generate_new_index(
    torch::Tensor in_edge_index,
    int32_t hot_emb_num
) {
  int src, dst;
  ...
  int map_length = mapping.size();

  torch::Tensor out_edge_index = torch::zeros({map_length},in_edge_index.options().dtype(at::kInt));
  auto out_edge_index_ptr = out_edge_index.accessor<int, 1>();
  

  // // generate the edge_list.
  for(int i = 0; i < map_length; i++){
    out_edge_index_ptr[i] = mapping[i] + hot_emb_num;
  }
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("generate_new_index", &rabbit_generate_new_index, "Get the reordered node id mapping: old_id --> new_id");
}