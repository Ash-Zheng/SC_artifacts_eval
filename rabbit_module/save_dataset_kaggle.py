...
if __name__ == "__main__":
    ...
    input_path = "/workspace/SC_artifacts_eval/processed_data/workspace/kaggle_workspace/output"
    ...
    access_list = []
    reordering_list = [2,3,11,15,20]
    for i in range(length):
        if i in reordering_list:
            if if_reorder == 1:
                x = torch.load("/workspace/SC_artifacts_eval/Access_Index/kaggle/access_index/access_index_"+ str(i) +"_new.pt").to(device)
            else:
                x = torch.load("/workspace/SC_artifacts_eval/Access_Index/kaggle/access_index/access_index_"+ str(i) +".pt").to(device)
        else:
            x = torch.load("/workspace/SC_artifacts_eval/Access_Index/kaggle/access_index/access_index_" + str(i) + ".pt").to(device)
    ...
    # ====================== save ========================
    if if_reorder == 1:
        torch.save(sparse, '/workspace/SC_artifacts_eval/Access_Index/kaggle/training_data/reordered_sparse.pt')
        torch.save(dense, '/workspace/SC_artifacts_eval/Access_Index/kaggle/training_data/reordered_dense.pt')
        torch.save(label, '/workspace/SC_artifacts_eval/Access_Index/kaggle/training_data/reordered_label.pt')
    else:
        torch.save(sparse, '/workspace/SC_artifacts_eval/Access_Index/kaggle/training_data/sparse.pt')
        torch.save(dense, '/workspace/SC_artifacts_eval/Access_Index/kaggle/training_data/dense.pt')
        torch.save(label, '/workspace/SC_artifacts_eval/Access_Index/kaggle/training_data/label.pt')
